...
//         include 'navg.php';
  ...
  <?php 
                 ...
                 <hr>
                  <?php 
                  $query="select * from tb_asignacion";
                  $resu = mysqli_query($objConexion, $query); 
                  $fechainfin=date('Y-m-d');
                  while($arry=mysqli_fetch_array($resu)){

                 
                   $fechafinal=$arry['final_normal'];
                
                //    echo $fechaatcual='2023-01-15';
                   $hola=2;
                   if ($fechafinal==  $fechainfin){
                       $quey="update tb_asignacion set estado='$hola' where final_normal='$fechainfin'";
                       $resultado = mysqli_query($objConexion, $quey); 
                   }
                }
