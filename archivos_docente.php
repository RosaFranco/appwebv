...
//  $chk = $objConexion->query("SELECT * FROM  upload where name = '$name' ")->rowCount();
  if($chk){
    ...
    // var_dump($tname);exit;
        $sql="SELECT * FROM  tb_upload2 where nombre = '$tname' ";
...
//    	$chk2 = $conn->query("SELECT * FROM  upload where name = '$tname' ")->rowCount();
 ...
 $move =  move_uploaded_file($temp,"archivos/".$fname);
 if($move){
      $sql="insert into tb_upload2(nombre,fnombre)values('$name','$fname')";
          $query= mysqli_query($objConexion,$sql);
// 	$query=$conn->query("insert into upload(name,fname)values('$name','$fname')");
	if($query){
	header("location:archivos_docente.php");
	}
	else{
	die(mysql_error());
	}
...
<html>
    <head>
        <title>Subir Archivos docente</title>
      ...
      <!--<h5 class="mt-4">Docentes</h5>-->
       <br>
                        <ol class="breadcrumb mb-4">
                            <h5 class="breadcrumb-item active">subir reporte docentes </h5>
                        </ol>
			...
			<?php
                          $sql="select * from tb_upload2 order by id desc";
          $query= mysqli_query($objConexion,$sql);
//			$query=$conn->query("select * from upload order by id desc");
			...
			<tr>
				...
				<td>
                                    <button class="btn btn-outline-info" title="Descargar"><a href="descargas.php?filename=<?php echo $name;?>&f=<?php echo $row['fnombre'] ?>"><i class="bi bi-arrow-down-circle"></i></a></button>
				</td>
			</tr>
			<?php }?>
		</table>
